...
describe ProjectsController, type: :controller do
  describe 'controller filters and includes' do
    it 'includes sign_in && entry_accessed filters' do
      expect(ProjectsController).to receive(:before_filter).with(:ensure_user_is_signed_in?)
      expect(ProjectsController).to receive(:before_filter).with(:entry_accessed?, {only: :index})
      load File.join(Rails.root, 'app/controllers/projects_controller.rb')
    end
  end
