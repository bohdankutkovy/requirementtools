...
;(function ($, window, document, undefined) {
	...
	/*
		...
		show nodes in a flattened structure
	*/
	Tree.prototype.setInitialStates = function (node, level) {
		...
		$.each(node.nodes, function checkStates(index, node) {
			...
			// show nodes in a flattened structure for use later
        ...
        readFiles: function (files) {
            ...
            readFile = function (i) {
                ...
                if ($preview.length > 0 && FileReader !== undefined) {
                    $status.html(msgLoading.replace('{show}', i + 1).replace('{files}', numFiles));
                    ...
                    reader.onloadend = function () {
                        msg = msgProgress
                            .replace('{show}', i + 1).replace('{files}', numFiles)
                    ...
                    reader.onprogress = function (data) {
                        if (data.lengthComputable) {
                            ...
                            msg = msgProgress.replace('{show}', i + 1).replace('{files}', numFiles)
...
//FileInput plugin definition
    ...
    $.fn.fileinputLocales.en = {
        ...
        msgLoading: 'Loading file {show} of {files} &hellip;',
        msgProgress: 'Loading file {show} of {files} - {name} - {percent}% completed.',
